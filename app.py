#!/usr/bin/env python3
# app.py - Servidor principal con bibliotecas por usuario y panel de administración
...
# Directorios base
# -------------------------------------------------------------------
BASE_MUSIC_DIR      = '/mnt/exfat_disco'
BASE_THUMBNAILS_DIR = os.path.expanduser('~/.cache/yt-music/thumbnails')
BASE_CACHE_DIR      = os.path.expanduser('~/.cache/yt-music/cache')
BASE_YOUTUBE_DIR    = os.path.expanduser('~/YouTube_Downloads')

for _d in (BASE_MUSIC_DIR, BASE_THUMBNAILS_DIR, BASE_CACHE_DIR, BASE_YOUTUBE_DIR):
...
# -------------------------------------------------------------------
def get_user_music_dir(user_id):
    path = os.path.join(BASE_MUSIC_DIR, f'user_{user_id}')
...
def api_current_user():
    ...
    mdir = get_user_music_dir(user['id'])
    user['song_count']      = len(lm.get_sorted_audio_files(mdir))
    user['storage_used_mb'] = round(lm.get_user_storage(mdir) / (1024 * 1024), 1)
...
def list_files():
    ...
    mdir     = get_user_music_dir(session['user_id'])
    tdir     = get_user_thumbnails_dir(session['user_id'])
    files, total, has_more = lm.get_page(page, per_page, mdir, tdir)
...
def serve_cover(filename):
    try:
        ...
        thumb = lm.ensure_thumbnail(filename, mdir, tdir)
...
def stream_audio(filename):
    filename = urllib.parse.unquote(filename)
    mdir     = get_user_music_dir(session['user_id'])
    response = send_from_directory(mdir, filename)
...
def download_file(filename):
    try:
        ...
        mdir      = get_user_music_dir(session['user_id'])
        filepath  = os.path.join(mdir, safe_name)
...
def upload_file():
    ...
    for file in request.files.getlist('files'):
        if file and file.filename:
            ...
            file.save(os.path.join(mdir, safe))
            lm.ensure_thumbnail(safe, mdir, tdir)
...
def search_files():
    ...
    tdir    = get_user_thumbnails_dir(session['user_id'])
    results = []
    for f in lm.get_sorted_audio_files(mdir):
        if query in f.lower():
            info = lm.get_audio_info_fast(f, mdir, tdir)
            if info:
                results.append(info)
...
def get_stats():
    mdir = get_user_music_dir(session['user_id'])
    return jsonify({
        'total_files': len(lm.get_sorted_audio_files(mdir)),
        'total_size': f"{lm.get_user_storage(mdir)/(1024**3):.2f} GB",
...
def start_thumbnail_processing():
    uid  = session['user_id']
    mdir = get_user_music_dir(uid)
    tdir = get_user_thumbnails_dir(uid)
    started = lm.start_thumbnail_processing(uid, mdir, tdir)
...
def run_download(download_id, url, download_type, effective_limit, user_id):
    ...
    # Subdirectorio único por descarga para evitar conflictos entre descargas concurrentes
    ...
    try:
            ...
            if d['status'] == 'downloading':
                ...
                if fname != counter['last_file']:
                    ...
                    if counter['total'] > 1:
                        ...
                        dl['message'] = f"Canción {counter['n']} de {counter['total']}"
                ...
                dl['message'] = 'Convirtiendo a MP3...'

        ydl_opts = {
            'format': 'bestaudio/best',
            'outtmpl': os.path.join(ydl_dir, '%(title)s.%(ext)s'),
            'writethumbnail': True,   # descargar carátula para embeberla
            'postprocessors': [
            ...
            try:
                with yt_dlp.YoutubeDL({'quiet': True, 'extract_flat': True, 'ignoreerrors': True}) as ydl_flat:
                    ...
                    if flat and 'entries' in flat:
                        ...
                        dl['total_items']  = counter['total']
                        dl['message'] = f"Descargando {counter['total']} canciones..."
            except Exception:
        ...
        # Mover MP3s al directorio del usuario
        moved = []
            ...
            if f.lower().endswith('.mp3'):
                ...
                try:
                    if os.path.exists(dst):
                        os.remove(dst)
                    shutil.move(src, dst)   # shutil.move funciona cross-device, os.rename no
                    moved.append(f)
                    lm.ensure_thumbnail(f, mdir, tdir)
                except Exception as e:
                    print(f"Error moviendo {f}: {e}")

        # Limpiar subdirectorio temporal
        try:
        ...
        dl.update({'status': 'completed', 'progress': 100, 'file_count': len(moved),
                   'message': f'{len(moved)} canción(es) descargada(s)',
...
def admin_list_users():
    ...
    for u in users:
        mdir = get_user_music_dir(u['id'])
        u['song_count'] = len(lm.get_sorted_audio_files(mdir))
        u['storage_mb'] = round(lm.get_user_storage(mdir) / (1024 * 1024), 1)
...
def admin_stats():
    ...
    for u in users:
        mdir = get_user_music_dir(u['id'])
        total_songs += len(lm.get_sorted_audio_files(mdir))
        total_size  += lm.get_user_storage(mdir)
    return jsonify({
...
if __name__ == '__main__':
    print("🎵 MusicCloud Server iniciado en http://localhost:5000")
